...
DROP VIEW CITY_V CASCADE CONSTRAINTS 
;

DROP VIEW COUNTRY_V CASCADE CONSTRAINTS 
;
...
DROP TABLE presentation_rrequest CASCADE CONSTRAINTS;
...
CREATE TABLE presentation_rrequest (
    id_room_requests NUMBER NOT NULL,
    podium_size      NUMBER
);

ALTER TABLE presentation_rrequest ADD CONSTRAINT presentation_rrequest_pk PRIMARY KEY ( id_room_requests );
...
ALTER TABLE organizers
    ADD CONSTRAINT organizers_organizers_fk FOREIGN KEY ( id_organizer1 )
        REFERENCES organizers ( id_organizer );
...
--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE presentation_rrequest
    ADD CONSTRAINT presentation_rrequest_room_request_fk FOREIGN KEY ( id_room_requests )
        REFERENCES room_requests ( id_room_request );
...
CREATE OR REPLACE VIEW CITY_V ( ID_CITY
   , name
   , ID_COUNTRY )
 AS SELECT
    ID_CITY
...
CREATE OR REPLACE VIEW COUNTRY_V ( ID_COUNTRY
   , name )
 AS SELECT
    ID_COUNTRY
   , name
...
CREATE OR REPLACE VIEW ROOM_REQUESTS_V ( id_organizer, id_location, min_capacity, reservation_start, reservation_end, reservation_length, vc_ready, podium_size ) AS
SELECT
    ...
    meeting_requests.vc_ready,
    presentation_requests.podium_size
FROM
         room_requests
...
CREATE OR REPLACE VIEW ROOMS_V ( name, capacity, id_location, availability_start, availability_end, vc_ready, podium_size ) AS
SELECT
    rooms.name,
    rooms.capacity,
    locations.id_location,
...
CREATE OR REPLACE TRIGGER fkntm_organizers BEFORE
    UPDATE OF id_organizer1 ON organizers
BEGIN
...
CREATE OR REPLACE TRIGGER arc_fkar_presentation_rrequest BEFORE
    INSERT OR UPDATE OF id_room_requests ON presentation_rrequest
    FOR EACH ROW
DECLARE
    d VARCHAR2(21);
BEGIN
    ...
    FROM
        room_requests a
    WHERE
        a.id_room_request = :new.id_room_requests;

    IF ( d IS NULL OR d <> 'PRESENTATION_RREQUEST' ) THEN
        raise_application_error(-20223, 'FK PRESENTATION_RREQUEST_ROOM_REQUEST_FK in Table PRESENTATION_RREQUEST violates Arc constraint on Table ROOM_REQUESTS - discriminator column type doesn''t have value ''PRESENTATION_RREQUEST'''
        );
    END IF;
